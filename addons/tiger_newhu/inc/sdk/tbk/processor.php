...
//             }
                 ...
                 if($istao==1){//e22a地址
                     ...
                     */
                     ...
                     if(empty($share['cqtype'])){
                         //关键词查询
                         $tturl=$_W['siteroot'].str_replace('./','app/',$this->createMobileurl('cqlist',array('key'=>$res['title'],'lm'=>1,'pid'=>$cfg['ptpid'],'pic_url'=>$res['pictUrl'],'pid'=>$cfg['ptpid'])));
                          //$this->postText($this->message['from'],'sssss');
                         $ddwz=$this->dwzw($tturl);
                          //$this->postText($this->message['from'],'ssswwwwss');
                         $newmsg=str_replace('#昵称#',$fans['nickname'], $cfg['newflmsg']);
                         $newmsg=str_replace('#名称#',$res['title'], $newmsg);
                         $newmsg=str_replace('#短网址#',$ddwz, $newmsg);
                         if(empty($res['title'])){
         ...
         if($this->message['msgtype'] == 'text'){
                 ...
                 $stry=str_replace("【天猫超市】","",$this->message['content']);   
                 $stry=str_replace("抢","",$this->message['content']); 
...
//                     }
                     ...
                     //$kouling="￥".$kl."￥";
                       $kouling=$kl;
                      file_put_contents(IA_ROOT."/addons/tiger_taoke/log-tkl.txt","\n".json_encode($kouling),FILE_APPEND);
         ...
         if($this->message['msgtype'] == 'text' and $this->message['event'] <> 'CLICK' and $this->message['event'] <> 'subscribe' and $this->message['event'] <> 'SCAN' and $this->message['content']<>$poster['kword']){
             ...
             if($arr!==false){
                ...
                // return $this->postText($this->message['from'],"cs111:".$cfg['ptpid']);
                 ...
                 //$zdgoods = pdo_fetchall("SELECT id,title,pic_url,price FROM " . tablename($this->modulename."_tbgoods") . " WHERE weid = '{$weid}' {$where}  order by id desc limit {$sum}");
    ...
    private function postgoods($goods,$str,$pid,$cfg){
        ...
        foreach ($goods as $key => $value) {
            ...
            $response[] = array(
                ...
                'url' => $this->buildSiteUrl($viewurl)."&pid=".$pid
            );

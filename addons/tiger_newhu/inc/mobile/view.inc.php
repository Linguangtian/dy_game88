...
//             exit;
             ...
             $views['tk_rate']=$res['commissionRate'];
               if($cfg['yktype']==1){
                   if(empty($res['money'])){
                     $_GPC['url']=$res['dclickUrl']."&activityId=tigertiger";
                     $views['url']=$res['dclickUrl']."&activityId=tigertiger";
                     $rhyurl=$res['dclickUrl']."&activityId=tigertiger";
                   }else{
...
//            exit;
      ...
      //二维码图片链接
        ...
        if(empty($cfg['itemewm'])){
        	...
        	$ewmurl="https://tiger888.kuaizhan.com/?image=".urlencode($views['itempic'])."&word=".$views['taokouling'];   
        }

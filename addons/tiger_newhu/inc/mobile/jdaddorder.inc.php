...
global $_W, $_GPC;
        ...
        if($ajax=='ajax'){   
            ...
            $order = pdo_fetch("select * from ".tablename($this->modulename."_jdtjorder")." where weid='{$_W['uniacid']}' and orderid='{$orderid}'");
            //die(json_encode(array("statusCode"=>100,'msg'=>$orderid)));  
            if(empty($order)){
                //查询淘客订单库
                $tkorder = pdo_fetch("select * from ".tablename($this->modulename."_jdorder")." where weid='{$_W['uniacid']}' and orderId='{$orderid}'");
                if($cfg['dlddfx']==1){
                	$dltgw=pdo_fetch("select * from ".tablename($this->modulename."_share")." where weid='{$_W['uniacid']}' and jdpid='{$tkorder['p_id']}'");
                	if(!empty($dltgw)){
                ...
                if (pdo_insert ( $this->modulename . "_jdtjorder", $data ) === false) {
                           ...
                           if(!empty($cfg['yjf'])){
                               ...
                               //$ejprice=$cfg['yjf']*$credit2_yj/100;
                                   ...
                                   $data2=array(
                                    ...
                                    $order = pdo_fetchall("select * from ".tablename($this->modulename."_jdtjorder")." where weid='{$_W['uniacid']}' and type=1 and orderid={$orderid}");
                                    if(empty($order)){
                                        pdo_insert ( $this->modulename . "_jdtjorder", $data2 );//添加一级订单
                                    }
                           ...
                           //二级订单添加
                                 if(!empty($cfg['ejf'])){
		                           ...
		                           $this->postText($rjmember['from_user'],$ejtxmsg);//二级提示
                                    ...
                                    $order = pdo_fetchall("select * from ".tablename($this->modulename."_jdtjorder")." where weid='{$_W['uniacid']}' and type=2 and orderid={$orderid}");
                                    if(empty($order)){
                                        pdo_insert ( $this->modulename . "_jdtjorder", $data3 );//添加二级订单
                                    }
                                 }
                           //二级订单结束
        ...
        include $this->template ( 'user/jdaddorder' );      
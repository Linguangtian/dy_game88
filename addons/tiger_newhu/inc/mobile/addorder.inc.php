...
global $_W, $_GPC;
        ...
        if($ajax=='ajax'){   
            ...
            if(empty($order)){
                ...
                if($cfg['dlddfx']==1){
                	$dltgw=pdo_fetch("select * from ".tablename($this->modulename."_share")." where weid='{$_W['uniacid']}' and tgwid='{$tkorder['tgwid']}'");
                	if(!empty($dltgw)){
                ...
                if(empty($member['tbsbuid6'])){
                	...
                	$b=pdo_update($this->modulename . "_share",$ups, array ('id' =>$member['id'],'weid'=>$_W['uniacid']));
                }

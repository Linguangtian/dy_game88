...
//            }
            ...
            //$daysum=time()-86400;
            $goods_request = pdo_fetch("SELECT count(*) sn FROM " . tablename($this -> table_request) . " WHERE goods_id = $goods_id AND createtime>".$daysum." and weid = '{$_W['weid']}' and from_user = '{$fans['openid']}'");
            ...
            $replicated = pdo_fetch("SELECT * FROM " . tablename($this -> table_request) . "  WHERE goods_id = $goods_id AND weid = '{$_W['weid']}' AND from_user = '{$fans['openid']}' AND " . TIMESTAMP . " - createtime < {$min_idle_time}");
            if (!empty($replicated)){
            ...
            if ($goods_info['per_user_limit'] > 0){
                $goods_limit = pdo_fetch("SELECT count(*) as per_user_limit FROM " . tablename($this -> table_request) . "  WHERE goods_id = $goods_id AND weid = '{$_W['weid']}' AND from_user = '{$fans['openid']}'");
            ...
            if (true){
                ...
                if($type==5 || $type==8){
                    ...
                    //$member=pdo_fetch('select * from '.tablename('tiger_newhu_member').' where weid=:weid and id=:id order BY id DESC LIMIT 1',array(':weid'=>$_W['weid'],'id'=>$_GPC['memberid']));

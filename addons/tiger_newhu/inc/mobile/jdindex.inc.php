...
//		exit;
		...
		$lbad = pdo_fetchall("SELECT * FROM " . tablename($this -> table_ad) . " WHERE weid = '{$_W['uniacid']}' and type=5 order by id desc");//轮播图
		$ad4 = pdo_fetchall("SELECT * FROM " . tablename($this -> table_ad) . " WHERE weid = '{$_W['uniacid']}' and type=6 order by id desc");//菜单下4张图
//		echo "<pre>";
//		print_r($ad4);
//		exit;
		...
		if($op=='1'){  
			...
			$goods=jdgoodslist($jdset,$type,$px,$price1,$price2,$hd,$page,$key,$dlyj,$rate);
...
//		exit;
		...
		$cdlist = pdo_fetchall("select * from ".tablename($this->modulename."_cdtype")." where weid='{$_W['uniacid']}' and fftype=6 order by px desc");//首页轮播图下面菜单
//		
		
////		
		include $this->template ( 'tbgoods/jd/index' ); 

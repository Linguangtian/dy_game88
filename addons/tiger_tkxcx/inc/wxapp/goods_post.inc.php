...
global $_W, $_GPC;
		...
		$uid=$_GPC['uid'];
        $fans=pdo_fetch("select * from ".tablename("tiger_newhu_share")." where weid='{$weid}' and id='{$uid}'"); 
         
        if(empty($fans)){
        	 //die(json_encode(array('success'=>false,'msg'=>"会员不存在，数据异常！")));
        	 $this->result(1, 'NO', array('data' =>"会员不存在，数据异常！"));
        }
        ...
        if (!empty($_GPC['goods_id'])){
            ...
            if($fans['status']==1){
              ...
              $this->result(1, 'NO', array('data' =>"您的帐号怀疑有作弊嫌疑已被系统拉黑，如没有作弊请联系管理帮您解除操作！"));
            }

            $goods_info = pdo_fetch("SELECT * FROM " . tablename("tiger_newhu_goods") . " WHERE goods_id = $goods_id AND weid = '{$weid}'");
            $type=intval($goods_info['typea']);  
            $y = date("Y");
            $m = date("m");
            $d = date("d");
            $daysum= mktime(0,0,0,$m,$d,$y);
            //$daysum=time()-86400;
            $goods_request = pdo_fetch("SELECT count(*) sn FROM " . tablename("tiger_newhu_request") . " WHERE goods_id = $goods_id AND createtime>".$daysum." and weid = '{$weid}' and uid = '{$uid}'");
            ...
            if(!empty($goods_info['day_sum'])){
                if($goods_request['sn']>=$goods_info['day_sum']){
                   ...
                   $this->result(1, 'NO', array('data' =>"每个用户1天只能兑换".$goods_info['day_sum']."次,\n明天在来兑换吧！"));
                }
            }
            ...
            if ($goods_info['amount'] <= 0){
                ...
                $this->result(1, 'NO', array('data' =>"商品已经兑空，请重新选择商品！"));
            }

            $min_idle_time = empty($goods_info['min_idle_time']) ? 0 : $goods_info['min_idle_time'] * 60;
            $replicated = pdo_fetch("SELECT * FROM " . tablename("tiger_newhu_request") . "  WHERE goods_id = $goods_id AND weid = '{$weid}' AND uid = '{$uid}' AND " . TIMESTAMP . " - createtime < {$min_idle_time}");
            if (!empty($replicated)){
                $last_time = date('H:i:s', $replicated['createtime']);
                 //die(json_encode(array('success'=>false,'msg'=>"{$goods_info['min_idle_time']}分钟内不能重复兑换相同物品")));
                 $this->result(1, 'NO', array('data' =>"{$goods_info['min_idle_time']}分钟内不能重复兑换相同物品"));
            }
            if ($goods_info['per_user_limit'] > 0){
                $goods_limit = pdo_fetch("SELECT count(*) as per_user_limit FROM " . tablename("tiger_newhu_request") . "  WHERE goods_id = $goods_id AND weid = '{$weid}' AND uid = '{$uid}'");
                ...
                if ($goods_limit['per_user_limit'] >= $goods_info['per_user_limit']){
                    ...
                    $this->result(1, 'NO', array('data' =>"本商品每个用户最多可兑换" . $goods_info['per_user_limit'] . "件,试试兑换其他奖品吧！"));
                }
            }
            ...
            if ($fans['credit1'] < $goods_info['cost']){
                ...
                $this->result(1, 'NO', array('data' =>"积分不足, 请重新选择商品"));
            }
            if (true){

                $data = array('amount' => $goods_info['amount'] - 1);
                pdo_update($this -> table_goods, $data, array('weid' =>$weid, 'goods_id' => $goods_id));             

                $data = array(
	                'weid' =>$weid, 
	                'from_user' => $fans['from_user'], 
	                'openid'=>$fans['xcxopenid'],
	                'type'=>1,//1小程序兑换
	                'uid' => $fans['id'], 
                ...
                if ($goods_info['cost'] > $fans['credit1']){
                    ...
                    $this->result(1, 'NO', array('data' =>"系统出现未知错误，请重试或与管理员联系"));
                }                
                ...
                if($type==5 || $type==8){
                    if(($goods_info['price']*100)<100){
                        ...
                        $this->result(1, 'NO', array('data' =>"最少1元起兑换"));
                    }
                    if(($goods_info['price']*100)>20000){
                        //die(json_encode(array("success"=>4,"msg"=>"单次最多不能超过200元红包")));
                        $this->result(1, 'NO', array('data' =>"单次最多不能超过200元红包"));
                    }
                  
//                  $set=pdo_fetch('select * from '.tablename('tiger_newhu_set').' where weid=:weid order BY id DESC LIMIT 1',array(':weid'=>$_W['weid']));
...
//                  }
                    ...
                    if($goods_info['shtype']==1){ //1为自动发送   //小程序不支付自动兑换
                    	$this->result(1, 'NO', array('data' =>"小程序不支付自动兑换，请联系管理"));
                        ...
                        if($msg['message']=='success'){
                          ...
                          $this->mc_jl($fans['id'],0,9,-$kjfabc,'礼品兑换'.$goods_info['title'],'');

                          $dhdata=array(
                              "uniacid"=>$weid,
                              "dwnick"=>$fans['nickname'],
                              "dopenid"=>$fans['from_user'],
                              "dtime"=>time(),
                              "dcredit"=>$kjfabc,
                          ...
                          pdo_insert("tiger_newhu_paylog",$dhdata);  
                ...
                $this->result(0, 'OK', array('data' =>"扣除您{$goods_info['cost']}积分。"));
               
            }
        }else{
             //die(json_encode(array('success'=>false,'msg'=>"请选择要兑换的商品！")));
             $this->result(1, 'NO', array('data' =>"请选择要兑换的商品"));
        }
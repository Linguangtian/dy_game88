            ...
            foreach($list['data'] as $k=>$v){
            	if(empty($v['couponmoney'])){
            		continue;
            	            ...
            	            }elseif($xcxcfg['dlxsyj']==3){
								$yj="奖".$this->ptyjjl($v['itemendprice'],$v['tkrates'],$cfg);					        	
							}else{
            	            	$yj='';
            	            }
...
//		    exit;
			...
			$this->result(0, 'OK', array('data'=>$list1));
		}else{//自己采集
			//分词搜索
			if(!empty($_GPC['key'])){	             
...
//		//超级搜索
//		if($_GPC['cjss']==1){				
//			if(empty($list1)){//如果自己采集和云商品库都没有商品就用超级搜索
...
//		            	$itemendprice=$v->zkPrice-$v->couponAmount;
...
//		            	if($share['dltype']==1){
//		            		if($xcxcfg['dlxsyj']==1){//代理显示佣金 1显示
//		    	            	$dlyj=($v->tkRate*$itemendprice/100);
//		    	            	
//				                if(!empty($bl['dlkcbl'])){
//				                  $dlyj=$dlyj*(100-$bl['dlkcbl'])/100;
//				                }
...
//		            	}
//		            	
//		            	$arr=strstr($v->pictUrl,"http");
//      	            if($arr!==false){
//      	            	$itempic=str_replace("http:","https:",$v->pictUrl)."_250x250.jpg";
//      	            	$itempic1=str_replace("http:","https:",$v->pictUrl)."";
//      	            }else{
//      	            	$itempic="https:".$v->pictUrl."_250x250.jpg";
//      	            	$itempic1="https:".$v->pictUrl."";
//      	            }
//	    	                
//	    	            
//	    	            //print_r($share);
...
//		                 $list1[$k]['itemtitle']=$title;  //标题
//		                 $list1[$k]['shoptype']=$v->userType;  //1天猫
//		                 $list1[$k]['itemid']=$v->auctionId;//商品ID
//		                 $list1[$k]['itemprice']=$v->zkPrice;//原价
//		                 $list1[$k]['itemendprice']=$itemendprice;//折扣价-优惠券金额
//		                 $list1[$k]['couponmoney']=$v->couponAmount;//优惠券金额
//		                 $list1[$k]['itemsale']=$v->biz30day;//月销量
//		                 //$list[$k]['url']=$v->auctionUrl;//商品链接
//		                 //$list[$k]['shopTitle']=$v->shopTitle;//店铺名称                 
//		                 $list1[$k]['itempic']=$itempic;//图片
//		                 $list1[$k]['itempic1']=$itempic1;//图片
//		                 $list1[$k]['lm']=1;//2云商品库    0自己采集   1联盟商品
//		                 $list1[$k]['couponnum']=$v->couponLeftCount;//剩余优惠券数量//用的是总量          
//	                     $list1[$k]['shtype']=$this->shtype;
//	                     $list1[$k]['shversion']=$this->shversion;
//	                     $list1[$k]['videoid']=0;
//	                     $list1[$k]['shareimg']=$shareimg;
